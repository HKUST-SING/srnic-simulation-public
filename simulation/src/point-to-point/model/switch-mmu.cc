...
namespace ns3 {
	...
	SwitchMmu::SwitchMmu(void){
		...
		// for active pkg dropping
		pkgnum = 0;

	}
	bool SwitchMmu::CheckIngressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize){ 
		// add Active Package Dropping
		// pkgnum += 1;
		// if(pkgnum > 1000000000){
		// 	pkgnum = 10;
		// }
		// if(pkgnum % 1000 == 0 && pkgnum < 100000){
		if (false){
			std::cout << "drop package actively " << pkgnum << "\n";
			return false;
		}
		else{
			if (psize + hdrm_bytes[port][qIndex] > headroom[port] && psize + GetSharedUsed(port, qIndex) > GetPfcThreshold(port)){
				...
				// printf("\n");
				// NS_LOG_INFO("Drop: queue: " << node_id << ", " << port << ": Headroom full");
				return false;
			}
			return true;
		}
	...
	bool SwitchMmu::CheckShouldPause(uint32_t port, uint32_t qIndex){
		bool res = !paused[port][qIndex] && (hdrm_bytes[port][qIndex] > 0 || GetSharedUsed(port, qIndex) >= GetPfcThreshold(port));
		// if (res==true){
		// 	std::cout << "PFC triggered" << "\n";
		// }
		// return res;
		return false;
	}
	...
	uint32_t SwitchMmu::GetPfcThreshold(uint32_t port){
		uint32_t res = (buffer_size - total_hdrm - total_rsrv - shared_used_bytes) >> pfc_a_shift[port]; // >>pfc_a_shift[port] : /8
		return res;
	}
