...
TRACE_OUTPUT_FILE mix/mix_{topo}_{trace}_{cc}_{retrans}{failure}{times}{ack_p}.tr
FCT_OUTPUT_FILE mix/fct_{topo}_{trace}_{cc}_{retrans}{failure}{times}{ack_p}.txt
PFC_OUTPUT_FILE mix/pfc_{topo}_{trace}_{cc}_{retrans}{failure}{times}{ack_p}.txt
...
QLEN_MON_FILE mix/qlen_{topo}_{trace}_{cc}_{retrans}{failure}{times}{ack_p}.txt
...
if __name__ == "__main__":
	...
	parser.add_argument('--retrans', dest='retrans', action='store', default='goBackN', help="retransmittion type, SR or GoBackN")
	parser.add_argument('--times', dest='times', action='store', default='1', help="repeat times")
	parser.add_argument('--ack_p', dest='ack_p', action='store', default='0', help="ack priority")
	...
	if args.ack_p == '0':
		...
		ack_p = "_a%s"%(args.ack_p)
		print("ack_p = %s"%(ack_p))
	
	config_name = "mix/config_%s_%s_%s_%s%s%s.txt"%(topo, trace, args.cc, args.retrans, times, ack_p)
	...
	if (args.cc.startswith("dcqcn")):
		...
		if args.cc == "dcqcn":
			config = config_template.format(bw=bw, trace=trace, topo=topo, cc=args.cc, mode=1, t_alpha=1, t_dec=4, t_inc=300, g=0.00390625, ai=ai, hai=hai, dctcp_ai=1000, has_win=0, vwin=0, us=0, u_tgt=u_tgt, mi=mi, int_multi=1, pint_log_base=pint_log_base, pint_prob=pint_prob, ack_prio=1, ack_p=ack_p, times=times, link_down=args.down, failure=failure, kmax_map=kmax_map, kmin_map=kmin_map, pmax_map=pmax_map, buffer_size=bfsz, enable_tr=enable_tr, retrans=args.retrans)
		...
		config_name = "mix/config_%s_%s_%s_%s%s%s.txt"%(topo, trace, cc, args.retrans, times, ack_p)
	...
	output_file = "outputs/output_%s_%s_%s_%s%s%s.txt"%(topo, trace, args.cc, args.retrans, times, ack_p)
	os.system("./waf --run 'scratch/third %s' > %s"%(config_name, output_file))