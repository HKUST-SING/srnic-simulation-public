...
double pause_time = 5, simulator_stop_time = 100.00;
std::string data_rate, link_delay;
std::string topology_file = "mix/fat.txt";
std::string flow_file = "mix/flowWebSearchLoad07Time001.txt";
std::string trace_file = "mix/trace.txt";
std::string trace_output_file = "mix/mix.tr";
std::string fct_output_file = "mix/fct.txt";
std::string pfc_output_file = "mix/pfc.txt";
std::string throughput_file = "mix/throughput.txt";

double alpha_resume_interval = 1, rp_timer = 900, ewma_gain = 0.00390625;
...
uint32_t has_win = 1;
uint32_t global_t = 1;
uint32_t mi_thresh = 0;
bool var_win = true, fast_react = true;
...
string qlen_mon_file = "mix/qlen.txt";

double custom_alpha = 1.0, custom_beta = 0.5;
uint32_t measurement_cycle = 1000;
...
void qp_finish(FILE* fout, Ptr<RdmaQueuePair> q){
	...
	fprintf(fout, "%08x %08x %u %u %lu %lu %lu %lu %u\n", q->sip.Get(), q->dip.Get(), q->sport, q->dport, q->m_size, q->startTime.GetTimeStep(), (Simulator::Now() - q->startTime).GetTimeStep(), standalone_fct, q->timeoutNums);
	fflush(fout);
...
{	
...
#endif
	{
		...
		while (!conf.eof())
		{
			...
			}else if (key.compare("CUSTOM_ALPHA") == 0){
				conf >> custom_alpha;
				std::cout << "CUSTOM_ALPHA\t\t\t\t" << custom_alpha << '\n';
			}else if (key.compare("CUSTOM_BETA") == 0){
				conf >> custom_beta;
				std::cout << "CUSTOM_BETA\t\t\t\t" << custom_beta << '\n';
			}else if (key.compare("MEASUREMENT_CYCLE") == 0){
				uint32_t v;
				conf >> v;
				measurement_cycle = v;
				std::cout << "MEASUREMENT_CYCLE\t\t\t" << measurement_cycle << "\n";
			}
			fflush(stdout);
		}
		conf.close();
	...
	NS_LOG_INFO("Explicitly create the channels required by the topology.");
	...
	for (uint32_t i = 0; i < link_num; i++)
	{
		...
		// NS_LOG_INFO("assign server IP" << i);
		// Assigne server IP
		...
		// NS_LOG_INFO("create a graph of the topology");
	...
	// install RDMA driver
	//
	NS_LOG_INFO("install RDMA driver.");
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 0){ // is server
			// create RdmaHw
			...
			rdmaHw->SetAttribute("CustomAlpha", DoubleValue(custom_alpha));
			rdmaHw->SetAttribute("CustomBeta", DoubleValue(custom_beta));
			rdmaHw->SetAttribute("MeasurementCycle", UintegerValue(measurement_cycle));
	...
	NS_LOG_INFO("set ACK priority on hosts");
	...
	NS_LOG_INFO("dump link speed to trace file");
	{
