...
if __name__=="__main__":
	...
	parser.add_argument('-l', dest='timeout_limit', action='store', type=int, default=-1, help="timeout times limit which record exceed will not be recorded")
	parser.add_argument('--topo', dest='topo', action='store', default="", help="Specify topology")
	parser.add_argument('--traffic', dest='traffic', action='store', default="", help="Specify traffic")
	parser.add_argument('--ccAndRetrans', dest="ccAndRetrans", action='store', default="", help="specify ALG list")
	...
	if args.topo == "":
		CCs = [] # write your own experiment setting
	else:
		topo = args.topo
		traffic = args.traffic
		CCs = []
		for i in args.ccAndRetrans.split("+"):
			CCs.append(topo+"_"+traffic+"_"+i)
		print(CCs)
	...
	for cc in CCs:
		file = "%s_%s.txt"%(args.prefix, cc) # your own file address
		# file = "../simulation/mix/fct.txt"
		if type == 0:
			if 'SR' in cc:
				cmd = "cat %s"%(file)+" | awk '{if ($4==100 && $6+$7<"+"%d"%time_limit+") {slow=$7/$8;print slow<1?1:slow, $5, $9}}' | sort -n -k 2"
			else:
		...
		a = output.split('\n')
		n = len(a)
		# timeout_limit = -1, means no timeout limit
	...
	if args.topo != "":
		res_file = "./result/files/fct_"+args.topo+"_"+args.traffic+"_"
		res_file += args.ccAndRetrans
		res_file += ".txt"
		print(res_file)
		resoutput = open(res_file, "w")

	for item in res:
		line = "%.3f %d"%(item[0], item[1])
		i = 1
		for cc in CCs:
			line += " %.3f %.3f %.3f %.3f"%(item[i+1], item[i+2], item[i+3], item[i+4])
			i += 5
		if args.topo != "":
			resoutput.write(line)
			resoutput.write("\n")
		print line
	
	if args.topo != "":
		resoutput.close()
	# show every cc's flow size
	# for item in res:
	# 	line = "%.3f"%(item[0])
	# 	i = 0
	# 	for cc in CCs:
	# 		line += " %d %.3f %.3f %.3f %.3f"%(item[i+1], item[i+2], item[i+3], item[i+4], item[i+5])
	# 		i += 5
	# 	print line
